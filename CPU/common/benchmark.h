...
pair<vector<data_type>, vector<TIMESTAMP>> read_data(const char* PATH, const count_type length, count_type* cnt) {
	vector<data_type> items;
	data_type it;
	...
	*cnt = 0;
	while (fread(&it, sizeof(data_type), 1, data) > 0 && fread(&timestamp, sizeof(TIMESTAMP), 1, data) > 0 && (*cnt) < length)
	{
		(*cnt)++;
		items.push_back(it);
		//timestamps.push_back(timestamp);
	}
	//double end = timestamps[(*cnt) - 1], start = timestamps[0];
	//cout << "duration:" << end - start << "s\n";
	fclose(data);
	/*
	node actual=new node[1500000];
	HashMap mp;
	for (int l = 0; l < (*cnt); ++l) {
	...
	delete[] actual;
	*/
	return make_pair(items, timestamps);
}
