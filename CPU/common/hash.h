...
unsigned int crc32(const unsigned char* buf, int len, unsigned int init = 0)
{
...
unsigned int truncate(const unsigned char* buf, int len, unsigned int slice = 2)
{
	unsigned int ret = 0;
	for (size_t i = len - slice; i < len; i++)
	{
		ret = (ret << 8) | buf[i];
	}
	return ret;
