...
#include <chrono>
using namespace std;
vector<string> folder = { "../../../datasets/MAC/" ,"../../../datasets/CAIDA/","../../../datasets/MAC/ACL/" };
...
uint32_t sum1 = 0;
void test_acl(const char* PATH)
{
	std::cout << "FIB Benchmark file=" << PATH << std::endl << std::endl;
	uint32_t cnt = 0;
	auto dataset = read_acl(PATH, cnt);
	cout << "Toal " << cnt << " items" << endl;
	int result = 0;
	ACL* acl = new ACL(dataset, result);
}
int main() {
	...
	test_acl((folder[0] + filenames[5]).c_str());
}