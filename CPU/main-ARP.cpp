...
const string filenames[] = { "complete_mac_table.txt" ,"trace3.dat","130000.dat","complete_arp_table.txt" };
void test(vector<pair<IP, MAC_ADD>>& dataset, uint32_t memory, int test_num,
	int mac_max_err, int cc_max_err, int hsh_max_err,
	double& mac_score, double& cc_score, double& hsh_score)
{
	ARP* mac;
	CuckooHash<IP, MAC_ADD>* cc;
	NormalHash<IP, MAC_ADD>* hsh;
	int cnt = 0;
	for (size_t round = 0; cnt < test_num; round++)
	{
		random_shuffle(dataset.begin(), dataset.end());
		int prelinmary = memory * 8 / MAC_ADD_LEN / 2.0 * 0.8, result = 0;
		mac = new ARP(vector<pair<IP, MAC_ADD>>(dataset.begin(), dataset.begin() + prelinmary), result, 2.0);
		if (result == -1)
		{
			delete mac;
			continue;
		...
		delete mac;
		cc = new CuckooHash<IP, MAC_ADD>(memory, MAC_ADD_LEN);
		...
		delete cc;
		hsh = new NormalHash<IP, MAC_ADD>(memory, MAC_ADD_LEN);
		for (i = 0; i < dataset.size(); i++)
		{
...
void test_ARP_memory(const char* PATH, vector<uint32_t> memorys, int test_num)
{
	std::cout << "FIB Benchmark file=" << PATH << std::endl << std::endl;
	uint32_t cnt = 0;
	vector<pair<IP, MAC_ADD>> dataset = read_arp(PATH, cnt);
	cout << "Toal " << cnt << " items" << endl;

	ofstream out1, out2;
	out1.open("../result/ARP_memory_Entry.csv");
...
void test_ARP_times(const char* PATH, vector<uint32_t> times, uint32_t memory, int test_num)
{
	std::cout << "FIB Benchmark file=" << PATH << std::endl << std::endl;
	uint32_t cnt = 0;
	vector<pair<IP, MAC_ADD>> dataset = read_arp(PATH, cnt);
	cout << "Toal " << cnt << " items" << endl;
	ofstream out1, out2;
	out1.open("../result/ARP_times_Entry.csv");
	out1 << "MEM(KB)" << "," << "UnifiedTable" << "," << "CuckooFilter" << "," << "Hash\n";
	for (auto& time : times)
	{
		out1 << time << ",";
...
int main() {
	...
	test_ARP_memory((folder[0] + filenames[3]).c_str(), vector<uint32_t>{1000, 2000, 4000, 8000, 16000}, 10);
	test_ARP_times((folder[0] + filenames[3]).c_str(), vector<uint32_t>{1, 4, 16, 64}, 4000, 10);
}